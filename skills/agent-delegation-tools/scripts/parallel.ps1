...
function Start-DelegationTask {
    ...
    $previousDelegate = $env:AGENT_PARALLEL_DELEGATE
    $previousParameters = $env:AGENT_PARALLEL_PARAMETERS
    try {
        $env:AGENT_PARALLEL_DELEGATE = $script:resolvedDelegatePath
        $env:AGENT_PARALLEL_PARAMETERS = $parameterFile
        if (-not $process.Start()) { throw "Failed to start parallel task '$($Task.Name)'." }
    }
    finally {
        $env:AGENT_PARALLEL_DELEGATE = $previousDelegate
        $env:AGENT_PARALLEL_PARAMETERS = $previousParameters
    }
...
for ($index = 0; $index -lt $taskItems.Count; $index++) {
    ...
    $taskType = if ($parameters.ContainsKey('TaskType')) { [string]$parameters['TaskType'] } else { 'analysis' }
    $sandbox = if ($parameters.ContainsKey('Sandbox')) {
        [string]$parameters['Sandbox']
    }
    elseif ($taskType -in @('implementation', 'scaffolding')) {
        'workspace-write'
    }
    else {
        'read-only'
