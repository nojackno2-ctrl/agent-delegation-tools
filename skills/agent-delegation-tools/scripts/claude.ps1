...
function Test-AuthenticationRequired {
    ...
    if ($ExitCode -ne 0 -and $Output -match '(?i)(not\s+logged\s+in|not\s+signed\s+in|login\s+required|log\s+in\s+required|authentication\s+required|unauthenticated|unauthorized|please\s+(?:log|sign)\s+in|missing\s+(?:credentials?|api\s+key)|invalid\s+(?:credentials?|api\s+key|authentication))') {
        return $true
    }
    try {
        $status = $Output | ConvertFrom-Json
        return $null -ne $status.loggedIn -and $status.loggedIn -eq $false
    }
    catch { return $false }
}

function Get-ClaudeLoginStatus {
    ...
    if ($extension -eq '.cmd' -or $extension -eq '.bat') {
        if (-not $env:ComSpec) { throw 'ComSpec is not set; cannot inspect Claude login status.' }
    ...
    try {
        if (-not $process.Start()) { throw 'Failed to inspect Claude login status.' }
        $stdoutTask = $process.StandardOutput.ReadToEndAsync()
        $stderrTask = $process.StandardError.ReadToEndAsync()
        if (-not $process.WaitForExit(30000)) {
            try { $process.Kill() } catch { }
            $null = $process.WaitForExit(10000)
            return [pscustomobject]@{ ExitCode = 124; Output = 'Claude login status check timed out.' }
        }
        $process.WaitForExit()
...
$loginStatus = Get-ClaudeLoginStatus -Executable $resolvedClaude
if (Test-AuthenticationRequired -ExitCode $loginStatus.ExitCode -Output $loginStatus.Output) {
    $guidance = "Claude CLI is not logged in. Run 'claude auth login' interactively, complete sign-in, then retry the delegated task."
    if ($resolvedOutFile) {
...
if (-not $timedOut -and (Test-AuthenticationRequired -ExitCode $exitCode -Output ($stdout + [Environment]::NewLine + $stderr))) {
    ...
    $stderr += $guidance + [Environment]::NewLine
}
