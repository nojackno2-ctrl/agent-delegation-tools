...
if (Test-AuthenticationRequired -ExitCode $exitCode -Output $combinedOutput) {
    $exitCode = 78
    $guidance = "AGY CLI is not authenticated. Run 'agy' interactively, complete sign-in, then retry the delegated task."
    if ($combinedOutput -and -not $combinedOutput.EndsWith([Environment]::NewLine)) {
        $combinedOutput += [Environment]::NewLine
    }
    $combinedOutput += $guidance + [Environment]::NewLine
