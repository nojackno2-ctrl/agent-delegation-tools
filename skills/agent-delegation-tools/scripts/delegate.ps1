...
param(
    ...
    # When omitted, implementation/scaffolding use workspace-write while analysis/review stay read-only.
...
if (-not $PSBoundParameters.ContainsKey('Sandbox')) {
    $Sandbox = if ($TaskType -in @('implementation', 'scaffolding')) { 'workspace-write' } else { 'read-only' }
}
...
function Invoke-WrapperProcess {
    ...
    $timedOut = $false
    $previousWrapper = $env:AGENT_DELEGATION_WRAPPER
    $previousParameters = $env:AGENT_DELEGATION_PARAMETERS
    try {
        $env:AGENT_DELEGATION_WRAPPER = $targetScript
        $env:AGENT_DELEGATION_PARAMETERS = $parameterFile
        ...
        $env:AGENT_DELEGATION_WRAPPER = $previousWrapper
        $env:AGENT_DELEGATION_PARAMETERS = $previousParameters
        $process.Dispose()
    }
}
